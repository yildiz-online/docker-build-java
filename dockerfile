...
RUN apt-get update && apt-get install -y -q wget unzip curl gnupg2 \
&& wget https://bitbucket.org/yildiz-engine-team/build-application-binaries/downloads/oraclejdk_linux-x64.zip\
&& wget https://bitbucket.org/yildiz-engine-team/build-application-binaries/downloads/apache-maven.zip \
&& unzip -q oraclejdk_linux-x64.zip \
&& rm oraclejdk_linux-x64.zip \
&& unzip -q apache-maven.zip \
&& rm apache-maven.zip\
&& chmod 777 /apache-maven/bin/mvn \
&& chmod 777 /oraclejdk_linux-x64/bin/java \
...
&& mvn dependency:get -DgroupId=org.jacoco -DartifactId=jacoco-maven-plugin -Dversion=0.8.1 \
